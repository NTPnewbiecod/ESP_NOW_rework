...
class ESPN ():
  ...
  def addPeer(self, ADDR :str,LMK_key :bytes= b'', Channel :int= 0,) -> None:
    ...
    """
    if self._get_peer_count() == self.espnow.MAX_TOTAL_PEER_NUM:
    ...
    try:
        self._EspNow.add_peer(bytes(int(i, 16) for i in ADDR.split(":")), LMK_key, Channel)
    except OSError:
        pass
  ...
  def send(self, msg :str, MAC_of_recv="FF:FF:FF:FF:FF:FF") ->None:
    ...
    """
    addr = bytes(int(i, 16) for i in MAC_of_recv.split(":"))
    try:
      self._EspNow.send(str(msg).encode("utf-8"), mac=addr)
    except OSError:
